...
type ConnectionPool struct {
	mu          sync.Mutex
	connections map[string]*clients.GmxClient
	limits      map[string]int
	usage       map[string]int
	rpcURLs     []string
}

func NewConnectionPool(rpcURLs []string) *ConnectionPool {
	return &ConnectionPool{
		connections: make(map[string]*clients.GmxClient),
...
func (cp *ConnectionPool) getClient(url string, pc *utils.GmxPriceCache) (*clients.GmxClient, error) {
	cp.mu.Lock()
	defer cp.mu.Unlock()

	client, exists := cp.connections[url]
	if !exists {
		var err error
		...
		cp.connections[url] = client
		cp.limits[url] = 1000 // Default rate limit, adjust as necessary
		cp.usage[url] = 0
	}
...
func (cp *ConnectionPool) getLeastUsedURL() string {
	cp.mu.Lock()
	defer cp.mu.Unlock()

	leastUsedURL := cp.rpcURLs[0]
	for _, url := range cp.rpcURLs {
		if cp.usage[url] < cp.usage[leastUsedURL] {
			leastUsedURL = url
		}
	}
	cp.usage[leastUsedURL]++
	return leastUsedURL
}
...
func main() {
	...
	cp := NewConnectionPool(rpcURLs)
	var wg sync.WaitGroup

	for _, user := range users {
		time.Sleep(2 * time.Second)
		wg.Add(1)
		url := cp.getLeastUsedURL()
		go startStream(user, url, pc, cp, &wg)
	}

	wg.Wait()
}

func startStream(user string, url string, pc *utils.GmxPriceCache, cp *ConnectionPool, wg *sync.WaitGroup) {
	defer wg.Done()
	client, err := cp.getClient(url, pc)
	if err != nil {
		fmt.Println("Error creating client:", err)
		return
	}
	client.StreamPositions(user, true, 10, true, true, sendCallBack)
}

func sendCallBack(newPositions []models.FuturesPosition, userId string, dataSource string) {
